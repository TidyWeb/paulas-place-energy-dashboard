...
export const defaults = {
  ...
  chargeEfficiency: 0.95, dischargeEfficiency: 0.95, inverterKw: 8, exportLimitKw: 3.68, gridCharge: true, peakExport: false,
...
const energyKeys = ['pvKwh','loadKwh','importKwh','exportKwh','directKwh','chargeKwh','gridChargeKwh','batteryKwh','batterySolarKwh','lossKwh','curtailKwh','importCost','exportRevenue','standingCost','baselineCost','ashpKwh','peakExportKwh','peakExportRevenue'];
  ...
  for (let i=0;i<n;i++) {
    ...
    if (state.gridCharge && m.period === 'cheap' && power > 0) {
      ...
      for(let j=i+1;j<Math.min(i+26,n);j++) {
        ...
        if(meta[j].period==='normal' && meta[j].hour<16) forecastSurplus += Math.max(0,Math.min(solar[j],inverter)-load[j]);
      }
      let target = Math.min(capacity, reserve + forecastNet/dischargeEfficiency);
      // Peak-export mode: fill with cheap power, leaving room for tomorrow's
      // forecast daytime solar surplus so solar is not pushed out to export.
      if (state.peakExport) target = Math.min(capacity, Math.max(target, capacity - forecastSurplus*chargeEfficiency));
    ...
    const solarExport = Math.min(surplus,positive(state.exportLimitKw));
    // Deliberate battery export in the peak window, keeping enough stored
    // energy (perfect forecast) to run the house until the next cheap window.
    let peakExport = 0;
    if (state.peakExport && m.period==='peak') {
      let need = 0;
      for (let j=i+1;j<Math.min(i+14,n) && meta[j].period!=='cheap';j++) need += Math.max(0,load[j]-Math.min(solar[j],inverter));
      const floor = reserve + need/dischargeEfficiency;
      const room = Math.min(Math.max(0,power-discharge), Math.max(0,inverter-availablePv-discharge), Math.max(0,positive(state.exportLimitKw)-solarExport));
      peakExport = Math.min(room, Math.max(0,soc-floor)*dischargeEfficiency);
      const share = soc > reserve ? clamp(solarSoc/(soc-reserve),0,1) : 0;
      solarSoc = Math.max(0,solarSoc-peakExport*share/dischargeEfficiency);
      soc -= peakExport/dischargeEfficiency;
    }
    const exportKwh = solarExport + peakExport;
    ...
    const metrics = {
      ...
      lossKwh:(solarCharge+gridCharge)*(1-chargeEfficiency)+(discharge+peakExport)*(1/dischargeEfficiency-1),
      curtailKwh:pv-availablePv+surplus-solarExport,importCost:imported*price,exportRevenue:exportKwh*exportPrice,
      peakExportKwh:peakExport,peakExportRevenue:peakExport*exportPrice,
